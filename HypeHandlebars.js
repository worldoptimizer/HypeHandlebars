...
if("HypeHandlebars" in window === false) window['HypeHandlebars'] = (function () {
	...
	function extendHype(hypeDocument, element, event) {
		 ...
		 *
 		 * - `updateOnHypeSceneLoad` can be either a `true` or a `function`. When set to `true` it runs either a plain `hypeDocument.updateHandlebars();` or the function you set to update Handlebars on scene load (HypeSceneLoad event), defaults to true
		 * - `updateOnHypeScenePrepareForDisplay` can be either a `true` or a `function`. When set to `true` it runs either a plain `hypeDocument.updateHandlebars();` or the function you set to update Handlebars on scene load (HypeSceneLoad event), defaults to true
		...
		if (!hypeDocument.getSymbolInstanceForElement) { 
			/**
			 * This helper does a backwards treewalk from the element it is started on and returns the symbolInstance if one is found. This function was originally written by Stephen Decker from Tumult Inc. When you use Hype Handlebars in conjunction with [Hype Symbol Cache](https://github.com/worldoptimizer/HypeSymbolCache) this function won't be defined by Hype Handlebars, but still work as it will be provided by Hype Symbol Cache offering the extended symbol functionality to Hype Handlebars as well.
			 *
